...
//            try db.run(imageTable.delete())
            ...
            _ = try db.run(insert)
            insert = imageTable.insert(id <- entry_id, image <- "https://firebasestorage.googleapis.com/v0/b/prodb-3f552.appspot.com/o/cover_art%2FAlways%20on%20my%20Mind.mp3.jpg?alt=media&token=3b248a51-4db1-45c3-a2fb-3ea5223f2940")
            _ = try db.run(insert)
            ...
            entry_id = UUID().asSQL()
            insert = happyTable.insert(id <- entry_id, trackName <- "Soul Searchin", producerName <- "sample", audioUrl <- "https://firebasestorage.googleapis.com/v0/b/prodb-3f552.appspot.com/o/tracks%2FSoul%20Searchin.mp3?alt=media&token=2097a44b-0a1a-4aef-971b-3c007f6e61a8", happyScore <- 0.25, aggScore <- 0.50)
            _ = try db.run(insert)
            ...
            entry_id = UUID().asSQL()
            insert = happyTable.insert(id <- entry_id, trackName <- "Time", producerName <- "sample", audioUrl <- "https://firebasestorage.googleapis.com/v0/b/prodb-3f552.appspot.com/o/tracks%2FTime.mp3?alt=media&token=0c0a96d4-53e8-49b6-b323-08a956af0ff2", happyScore <- 0.55, aggScore <- 0.20)
            _ = try db.run(insert)
